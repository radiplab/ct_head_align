...
def align(nii_path, output_path, debug=False):
    ...
    input_config.read('./paths.ini')
    paths_config = input_config['paths']
    nb_model_path = paths_config['nb_model_path']
    rc_model_path = paths_config['rc_model_path']
    lc_model_path = paths_config['lc_model_path']
    pitch_degrees = None
    roll_degrees = None
    yaw_degrees = None
    transforms = None
    ...
    if orig_slice_thickness <= 1.0:
        ...
        nb_label_image = predict_low_res_interpolate(input_sitk=aligned_sitk, model_path=nb_model_path, spacing=(1.5,1.5,1.5), roi=(96,96,96), num_labels=1, verbose=True)
        rc_label_image = predict_low_res_interpolate(input_sitk=aligned_sitk, model_path=rc_model_path, spacing=(1.5,1.5,1.5), roi=(96,96,96), num_labels=1, verbose=True)
        lc_label_image = predict_low_res_interpolate(input_sitk=aligned_sitk, model_path=lc_model_path, spacing=(1.5,1.5,1.5), roi=(96,96,96), num_labels=1, verbose=True)
        ...
        nb_mm = tuple(np.multiply(nb_index, aligned_sitk.GetSpacing()))
        ...
        rc_mm = tuple(np.multiply(rc_index, aligned_sitk.GetSpacing()))
        ...
        lc_mm = tuple(np.multiply(lc_index, aligned_sitk.GetSpacing()))

        # Calculate the rotation center
        transforms = []
        x, y, z = aligned_sitk.GetSize()
        rotation_center_index = (int(np.ceil(x/2)), int(np.ceil(y/2)), int(np.ceil(z/2)))
        rotation_center_mm = tuple(np.multiply(rotation_center_index, aligned_sitk.GetSpacing()))
        ...
        rotate_transform = get_rotate_transform(aligned_sitk, angle=pitch_degrees, pitch=True)
        ...
        # Create transform for rotation
        rotate_transform = get_rotate_transform(aligned_sitk, angle=yaw_degrees, yaw=True)
        transforms.append(rotate_transform)
        ...
        # Create transform for rotation
        rotate_transform = get_rotate_transform(aligned_sitk, angle=roll_degrees, roll=True)
        transforms.append(rotate_transform)
        ...
        # Apply transforms
        # **********************
        aligned_sitk = composite(aligned_sitk, transforms, interpolation=sitk.sitkBSpline)

        sitk.WriteImage(aligned_sitk, output_path)
...
def get_angle_to_match_y(xy_coords1, xy_coords2, rotation_center):
    ...
    # Find the smallest angle of rotation - a little tricky since 'min' may return a really large negative number
